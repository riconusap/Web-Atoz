...
?>
	...
	if (isset($_GET['username'])) {
		...
		$select = mysqli_query($konek, "SELECT * FROM user WHERE email='$id'") or die(mysqli_error($konek));
		...
		if (in_array($fileActualExt, $allowedExt)) {
			...
			if ($fileError === 0) {
				if ($fileSize < 10485760) {
					...
					mysqli_query($konek, "UPDATE user SET foto='$fileNameNew' WHERE id_user='$id'") or die(mysqli_error($konek));
					echo '<script>
    ...
    if (isset($_POST['profil_submit'])){
        ...
        if($email == $data['email']){
            mysqli_query($konek, "UPDATE user SET email='$email', nama='$name', instagram='$instagram' WHERE id_user='$id'") or die(mysqli_error($konek));
        echo '<script>
                        alert("Berhasil Mengubah Data.");
                        document.location = "index.php?page=tampil_profil";
                        </script>';
        } else {
            mysqli_query($konek, "UPDATE user SET username='$username', nama='$name', instagram='$instagram' WHERE id_user='$id'") or die(mysqli_error($konek));
            echo '<script>
            ...
            <form action="index.php?page=tampil_profil" method="post" enctype="multipart/form-data">
                <div class="card mb-3">
                    <div class="card-body text-center shadow">
                        <img class="rounded-circle shadow-sm mb-3 mt-4" src="<?php echo $hostname ?>assets/img/avatars/<?php echo $data['foto'] ?>" width="160" height="160" />
                        <div class="mb-3">
                    ...
                    <div class="col" style="height: 100%;width: 100px;">
                        <div class="card shadow mb-3">
                            ...
                            <form action="index.php?page=tampil_profil" method="post" enctype="multipart/form-data">
                                    <div class="form-row">
                                        <div class="col">
                                            <div class="form-group"><label for="username"><strong>Email</strong></label><input maxlength="8" type="text" class="form-control" name="username" value="<?= $data['email'] ?>" /></div>
                                        </div>
                                        <?= $instagram_edit; ?>
                                    </div>
                                    <div class="form-row">
                                        <div class="col">
                                            <div class="form-group"><label for="first_name"><strong>Nama</strong></label><input type="text" class="form-control" name="name" value="<?= $data['nama'] ?>" /></div>
                                        </div>
                                    </div>
